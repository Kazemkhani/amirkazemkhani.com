...
/* ─────────────────────────────────────────────
   Section blocks with enhanced scroll animations
   ───────────────────────────────────────────── */
  ...
  return (
    <div ref={ref} className="mt-20 mb-8 first:mt-0">
      ...
      <h2 className="font-display text-2xl md:text-3xl font-bold text-foreground">
        {isInView ? (
          <WordReveal text={section.content} delay={0.3} />
        ) : (
          <span className="opacity-0">{section.content}</span>
        )}
      </h2>
    </div>
  );
  ...
  return (
    <blockquote ref={ref} className="my-14 relative">
      ...
      <motion.div
        ...
        style={{
          background: 'linear-gradient(180deg, #c9a84c, rgba(201,168,76,0.3), transparent)',
        }}
      />
      ...
      <motion.div
        ...
        className="absolute -left-20 top-1/2 -translate-y-1/2 w-40 h-40 rounded-full bg-gold-500/[0.03] blur-[60px] pointer-events-none"
      />
    </blockquote>
  );
}
...
export default function ArticlePage({ slug }: { slug: string }) {
  ...
  const gridY = useTransform(heroProgress, [0, 1], ['0%', '15%']);
    ...
    <div ref={containerRef} className="min-h-screen bg-background">
      ...
      >
        ...
        <motion.div className="absolute inset-0" style={{ y: heroY, scale: heroScale }}>
          {/* Ambient blur orbs that drift */}
          <motion.div
            className="absolute top-1/4 left-1/4 w-96 h-96 rounded-full bg-gold-500/5 blur-[120px]"
            animate={{ x: [0, 30, 0], y: [0, -20, 0] }}
            transition={{ duration: 8, repeat: Infinity, ease: 'easeInOut' }}
          />
          <motion.div
            className="absolute bottom-1/4 right-1/3 w-80 h-80 rounded-full bg-white/[0.03] blur-[100px]"
            animate={{ x: [0, -25, 0], y: [0, 15, 0] }}
            transition={{ duration: 10, repeat: Infinity, ease: 'easeInOut' }}
          />
          <motion.div
            className="absolute top-1/2 right-1/4 w-64 h-64 rounded-full bg-gold-500/[0.03] blur-[80px]"
        ...
        <motion.button
          ...
          className="absolute bottom-8 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-2 text-white/30 hover:text-gold-500 transition-colors cursor-pointer"
        >
